...
import {
  ...
  Trash2, Plus, UserRound, ShieldCheck,
} from 'lucide-react';
...
const CompleteRegistration = () => {
  ...
  // ── Render ─────────────────────────────────────────────────────────────────
  return (
    <div className="min-h-screen bg-[var(--color-bg-subtle)] py-10 px-4 flex flex-col items-center">
          ...
          <div className="p-8 md:p-10">
            ...
            <div className="mb-8">
              <h1 className="text-2xl font-bold text-[var(--color-text-primary)]">
                {step === 1 ? t('completeReg.step1Title') :
                 step === 2 ? t('completeReg.step2Title') :
                 t('completeReg.step3Title')}
              </h1>
              <p className="text-sm text-[var(--color-text-muted)] mt-1.5">
                {step === 1 ? t('completeReg.step1Subtitle') :
                 step === 2 ? t('completeReg.step2Subtitle') :
                 t('completeReg.consentSubtitle')}
              </p>
            </div>
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="flex flex-col gap-1.5">
                    <Label htmlFor="org_type" className="text-sm font-medium text-[var(--color-text-secondary)]">
                      {t('completeReg.companyType')} <span className="text-[var(--color-danger)]">*</span>
                    </Label>
                    <Select value={form.org_type} onValueChange={v => setField('org_type', v)}>
                      <SelectTrigger className={`h-11${(!form.org_type && touched) ? ' cr-input-err' : ''}`}>
                    ...
                    {!form.org_type && touched && <p className="cr-err">{t('completeReg.requiredField')}</p>}
                  </div>

                  <div className="flex flex-col gap-1.5">
                    <Label htmlFor="director_name" className="text-sm font-medium text-[var(--color-text-secondary)]">
                      {t('completeReg.directorName')} <span className="text-[var(--color-danger)]">*</span>
                    </Label>
                    <Input
                      id="director_name"
                    ...
                    />
                    {!form.director_name.trim() && touched && <p className="cr-err">{t('completeReg.requiredField')}</p>}
                  </div>
                </div>

                {/* Legal address — full width */}
                <div className="flex flex-col gap-1.5">
                  <Label htmlFor="legal_address" className="text-sm font-medium text-[var(--color-text-secondary)]">
                    {t('completeReg.legalAddress')} <span className="text-[var(--color-danger)]">*</span>
                  </Label>
                  <Input
                    id="legal_address"
                  ...
                  />
                  {!form.legal_address.trim() && touched && <p className="cr-err">{t('completeReg.requiredField')}</p>}
                </div>
                ...
                <button
                  ...
                  className={`w-full flex items-center justify-center gap-2 py-3 rounded-xl text-sm font-semibold transition-colors
                    ${step1Valid
                      ...
                      : 'bg-[var(--color-bg-muted)] text-[var(--color-text-placeholder)] cursor-not-allowed'
                    }`}
                >
            ...
            {step === 2 && (
              <form onSubmit={handleStep2Next} className="space-y-6" noValidate>
                ...
                <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-bg-subtle)] p-6 space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="flex flex-col gap-1.5">
                      <Label htmlFor="dir-phone" className="text-sm font-medium text-[var(--color-text-secondary)]">
                        {t('completeReg.directorPhone')} <span className="text-[var(--color-danger)]">*</span>
                      </Label>
                      <Input
                        id="dir-phone"
                        type="tel"
                    ...
                    <div className="flex flex-col gap-1.5">
                      <Label htmlFor="dir-email" className="text-sm font-medium text-[var(--color-text-secondary)]">
                        {t('completeReg.directorEmail')} <span className="text-[var(--color-danger)]">*</span>
                      </Label>
                      <Input
                        id="dir-email"
                        type="email"
              ...
              <form onSubmit={handleSubmit} className="space-y-6" noValidate>
                ...
                <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-bg-subtle)] p-6 space-y-4">
                  ...
                  <div className="cr-legal-scroll">
                    <p>Настоящая Политика конфиденциальности определяет порядок обработки и защиты персональных данных пользователей электронной платформы HubContract, предназначенной для проведения электронных государственных и коммерческих закупок.</p>
                    <p>Администратор Платформы уважает конфиденциальность всех пользователей и обязуется защищать их персональные данные в соответствии с законодательством Республики Казахстан, включая Закон РК «О персональных данных и их защите».</p>
                    <p><strong>Собираемые данные:</strong> При регистрации и использовании Платформы собираются следующие данные: ФИО, ИИН/БИН, адрес электронной почты, номер телефона, юридический адрес, контактные данные представителя, банковские реквизиты, технические данные (IP-адрес, тип браузера), а также загружаемые документы (лицензии, сертификаты, банковские гарантии).</p>
                    <p><strong>Цели обработки:</strong> Данные используются для идентификации пользователей, проведения закупочных процедур, верификации участников, формирования договоров, направления уведомлений и обеспечения безопасности Платформы.</p>
                    <p><strong>Срок хранения:</strong> Персональные данные хранятся в течение 5 лет с момента последней активности, если иное не предусмотрено законодательством РК.</p>
                    <p><strong>Права пользователя:</strong> Вы вправе запросить доступ к своим данным, потребовать их исправления или удаления, отозвать согласие на обработку, а также обратиться с жалобой в уполномоченный орган по защите персональных данных.</p>
                    <p><strong>Передача третьим лицам:</strong> Данные не передаются третьим лицам без вашего согласия, за исключением случаев, предусмотренных законодательством РК или необходимых для исполнения договорных обязательств.</p>
                  </div>
                ...
                <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-bg-subtle)] p-6 space-y-4">
                  ...
                  <div className="cr-legal-scroll">
                    <p>Настоящий документ определяет ограничения ответственности оператора электронной платформы HubContract при использовании сервиса пользователями.</p>
                    <p><strong>Информация на Платформе:</strong> Администратор Платформы не несёт ответственности за точность, полноту и актуальность информации, размещаемой пользователями; содержание тендерной документации, предоставленной заказчиками; качество товаров/работ/услуг, указанных в заявках поставщиков; подлинность документов, загруженных пользователями.</p>
                    <p><strong>Технические аспекты:</strong> Администратор не гарантирует бесперебойную работу Платформы 24/7, отсутствие ошибок в программном обеспечении, совместимость со всеми устройствами и браузерами, абсолютную защиту от кибератак.</p>
                    <p><strong>Финансовые риски:</strong> Платформа является инструментом для проведения закупочных процедур. Все финансовые решения принимаются участниками самостоятельно. Администратор не несёт ответственности за финансовые потери, возникшие в результате использования Платформы.</p>
                    <p><strong>Форс-мажор:</strong> Администратор освобождается от ответственности за нарушение условий обслуживания вследствие обстоятельств непреодолимой силы (стихийные бедствия, действия государственных органов, сбои в работе интернет-провайдеров).</p>
                    <p><strong>Ограничение ответственности:</strong> В максимально допустимой законом мере совокупная ответственность Администратора перед пользователем не может превышать сумму комиссионного вознаграждения, уплаченного пользователем за последние 12 месяцев.</p>
                  </div>
                  <label className="cr-agree-row">
                    <input
                      type="checkbox"
                ...
                <div className="flex items-center justify-between gap-4 pt-2">
                  ...
                  <button
                    ...
                    style={{ opacity: step3Valid ? 1 : 0.5, cursor: step3Valid ? 'pointer' : 'not-allowed' }}
                  >
                    {loading
                      ? <><Loader2 size={16} className="cr-spin" /> {t('completeReg.submit')}…</>
